...
server.tool(
  ...
  async ({ title, content, tags = [], folder }) => {
    try {
      const success = notesManager.createNote(title, content, tags, folder);
      if (!success) {
        const hint = folder
          ? ` Folder "${folder}" may not exist.`
          : "";
        return {
          content: [{ type: "text", text: `Failed to create note.${hint} Please check your Apple Notes configuration.` }],
          isError: true,
        };
      }
      ...
      return {
        content: [{ type: "text", text: `Error creating note: ${error instanceof Error ? error.message : "Unknown error"}` }],
        isError: true,
      };
    }
  }
...
server.tool(
  ...
  async ({ query }) => {
    try {
      ...
      return {
        content: [{ type: "text", text: `Error searching notes: ${error instanceof Error ? error.message : "Unknown error"}` }],
        isError: true,
      };
    }
  }
...
server.tool(
  ...
  async ({ title }) => {
    try {
      ...
      return {
        content: [{ type: "text", text: `Error retrieving note: ${error instanceof Error ? error.message : "Unknown error"}` }],
        isError: true,
      };
    }
  }
